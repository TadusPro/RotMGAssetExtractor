namespace RotMGAssetExtractor.UnityExtractor.resextractor
{
    ...
    {
        private static readonly Dictionary<int, ClassIDType> EnumByIndex = new Dictionary<int, ClassIDType>();

        static ClassIDTypeHelper()
        {
            foreach (ClassIDType type in Enum.GetValues(typeof(ClassIDType)))
            {
                EnumByIndex.Add((int)type, type);
            }
        }

        public static ClassIDType GetClassIDTypeByIndex(int index)
        {
            if (EnumByIndex.TryGetValue(index, out ClassIDType type))
            {
                return type;
            }
        ...
        {
            if (EnumByIndex.TryGetValue(index, out ClassIDType result))
            {
                return result;
            }
            return ClassIDType.UnknownType; // Default if not found
        }
